...
/**
 `MGLRasterSource` is a map source that supplies raster image tiles to be shown
 on the map. The location of and metadata about the raster tiles are defined by
 either an `MGLTileSet` object or an external TileJSON resource. A raster source
 is added to an `MGLStyle` object along with an `MGLRasterStyleLayer` object.
 Use a raster style layer to control the appearance of content supplied by the
 raster source.
 */
